    ...
    for k,v in ecs_dict.items():
        ...
        data = {
            ...
            'Meta': {
                ...
                'exp': v.get('exp'),
                'stat': stat,
                # 以下字段为扩展字段，与 sync_cmdb_agent 保持一致
                'hostname': v.get('hostname'),
                'SerialNumber': v.get('SerialNumber'),
                'zone_id': v.get('zone_id'),
                'ZoneId': v.get('ZoneId'),
                'OSName': v.get('OSName'),
                'OSType': v.get('OSTypeFull') if v.get('OSTypeFull') is not None else v.get('OSType'),
                'Memory': v.get('Memory'),
                'vcpu': v.get('vcpu'),
                'InstanceType': v.get('InstanceType'),
                'CreationTime': v.get('CreationTime'),
                'ExpiredTime': v.get('ExpiredTime'),
                'ResourceGroupId': v.get('ResourceGroupId'),
                'EipAddress': v.get('EipAddress'),
                'PublicIpAddress': v.get('PublicIpAddress'),
                'InnerIpAddress': v.get('InnerIpAddress'),
                'VpcAttributes': v.get('VpcAttributes'),
                'InstanceNetworkType': v.get('InstanceNetworkType'),
                'ServerStatus': v.get('ServerStatus'),
